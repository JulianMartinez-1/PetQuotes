...
export function Card({ children, className, variant = "default", interactive = false }: CardProps) {
  ...
  return <div className={cn("rounded-2xl p-6 transition-all duration-300", variants[variant], interactive && "hover:border-secondary/60 hover:shadow-lg hover:-translate-y-1 cursor-pointer", className)}>{children}</div>;
}
