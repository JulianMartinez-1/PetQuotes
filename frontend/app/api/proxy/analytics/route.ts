...
async function proxyGet(request: NextRequest, endpoint: string): Promise<NextResponse> {
  ...
  const upstreamUrl = `${API_GATEWAY_URL}/api/analytics/${endpoint}${url.search}`;
...
export async function GET(request: NextRequest) {
  return proxyGet(request, "summary");
}
